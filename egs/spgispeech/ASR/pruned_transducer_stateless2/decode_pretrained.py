...
from asr_datamodule import SPGISpeechAsrDataModule, load_ec53_cuts
from beam_search import (
    ...
    modified_beam_search_rnnlm_shallow_fusion_biased,
)
from train import get_params, get_transducer_model

from icefall import BiasedNgramLm, NgramLm
...
# From: egs/librispeech/ASR/lstm_transducer_stateless2/train.py
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=0.01,
        help="""
        Used only when --decoding_method is modified_beam_search_rnnlm_shallow_fusion_biased.
...
def decode_one_batch(
    ...
    biased_lm: Optional[BiasedNgramLm] = None,
    biased_lm_scale: float = 1.0,
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        for hyp in sp.decode(hyp_tokens):
            hyps.append(hyp.split())
    elif params.decoding_method == "modified_beam_search_rnnlm_shallow_fusion_biased":
        hyp_tokens = modified_beam_search_rnnlm_shallow_fusion_biased(
            model=model,
            encoder_out=encoder_out,
            encoder_out_lens=encoder_out_lens,
            beam=params.beam_size,
            sp=sp,
            biased_lm=biased_lm,
            biased_lm_scale=biased_lm_scale,
            rnnlm=rnnlm,
            rnnlm_scale=rnnlm_scale,
        )
        for hyp in sp.decode(hyp_tokens):
...
def decode_dataset(
    ...
    word_table: Optional[k2.SymbolTable] = None,
    decoding_graph: Optional[k2.Fsa] = None,
    biased_lm: Optional[NgramLm] = None,
    biased_lm_scale: float = 1.0,
    ngram_lm: Optional[BiasedNgramLm] = None,
    ngram_lm_scale: float = 1.0,
    rnnlm: Optional[RnnLmModel] = None,
    rnnlm_scale: float = 1.0,
...
def main():
    ...
    params.suffix += f"-{params.tokens_ngram}-ngram-lm-scale-{params.ngram_lm_scale}"
    if "rnnlm" in params.decoding_method:
        params.suffix += f"-rnnlm-lm-scale-{params.rnn_lm_scale}"
    if "biased" in params.decoding_method:
        params.suffix += f"-biased-lm-scale-{params.biased_lm_scale}"
    ...
    if "ngram" in params.decoding_method or "LODR" in params.decoding_method:
        ...
        #     is_binary=False,
        # )
        lm_filename = f"{params.tokens_ngram}gram.fst.bin"
        logging.info(f"lm filename: {lm_filename}")
        ngram_lm = NgramLm(
            str(params.lang_dir / lm_filename),
            backoff_id=params.backoff_id,
    ...
    if "biased" in params.decoding_method:
        ...
        lm_filename = "context_graph.fst.bin"
        logging.info(f"biased lm filename: {lm_filename}")
        biased_lm = BiasedNgramLm(
            str(params.lang_dir / lm_filename),
            backoff_id=params.backoff_id,
            is_binary=True,
        )
        logging.info(f"num states: {biased_lm.lm.num_states}")
        logging.info(
            f"num arcs:   {sum([biased_lm.lm.num_arcs(state) for state in kaldifst.StateIterator(biased_lm.lm)])}"
        )
        biased_lm_scale = params.biased_lm_scale
    else:
        biased_lm = None
        biased_lm_scale = None
