...
from alignment import (
    force_alignment, 
    force_alignment_batch, 
    get_word_starting_frames, 
    get_word_starting_frames2,
)
...
def get_word_alignment(hyp_raw, hyp_ts):
    ts = []
    if len(hyp_raw) == 0:
        return ts

    for token, t in zip(hyp_raw, hyp_ts):
        if token[0] == "▁":
            ts.append(t)
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            for cut_id, hyp, ref_text in zip(cut_ids, hyps, texts):
                hyp_words, hyp_raw, hyp_ts = hyp
                hyp_ts_words = get_word_alignment(hyp_raw, hyp_ts)

                ref_words = text_normalize(ref_text).split()
                hyp_words = text_normalize(" ".join(hyp_words)).split()
                this_batch.append((cut_id, ref_words, hyp_words, hyp_ts_words))
...
def compute_alignments(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        if batch_idx % log_interval == 0:
            ...
            logging.info(f"batch {batch_str}, cuts processed until now is {len(ali_list)}")
...
def save_results(
    ...
    for key, results in results_dict.items():
        ...
        ts_path = params.res_dir / f"timestamps-{test_set_name}-{params.suffix}.txt"
        results = sorted(results)

        _results = results
        results = [(cut_id, ref_words, hyp_words) for (cut_id, ref_words, hyp_words, hyp_ts_words) in _results]
        timestamps = [(cut_id, hyp_ts_words) for (cut_id, ref_words, hyp_words, hyp_ts_words) in _results]

        store_timestamps(filename=ts_path, timestamps=timestamps)
...
def add_model_arguments2(parser):
    # parser.add_argument(
    #     "--out-dir",
    #     type=str,
    #     required=True,
...
def main():
    parser = get_parser()
    parser = add_model_arguments2(parser)
    ...
    if args.part is not None:
        ...
        train_cuts = CutSet.from_cuts([c for c in train_cuts if int(c.id.split("-")[-1].split("_")[0]) % n_parts == part])
