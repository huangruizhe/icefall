...
def decode_one_batch(
    ...
    if not model.no_encoder_biasing:
        encoder_biasing_out, attn = model.encoder_biasing_adapter[-1].forward(encoder_out, contexts_h, contexts_mask, need_weights=False)
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        hyp_tokens = modified_beam_search(
            ...
            get_topk_hyps=get_topk_hyps,
        )
        if get_topk_hyps != False:
            for topk_hyp_tokens in hyp_tokens:
                hyps.append([hyp.split() for hyp in sp.decode(topk_hyp_tokens)])
        else:
            for hyp in sp.decode(hyp_tokens):
                hyps.append(hyp.split())
...
def decode_dataset(
    ...
    get_topk_hyps = False  # None means "all", False means False, an integer x means top-x
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            if get_topk_hyps != False:
                for cut_id, topk_hyp_words, ref_text in zip(cut_ids, hyps, texts):
                    for ith, hyp_words in enumerate(topk_hyp_words):
                        ref_words = text_normalize(ref_text).split()
                        hyp_words = text_normalize(" ".join(hyp_words)).split()
                        this_batch.append((f"{cut_id}---{ith}", ref_words, hyp_words))
            else:
