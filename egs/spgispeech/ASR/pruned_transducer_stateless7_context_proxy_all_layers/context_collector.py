...
def normalize_text(text: str) -> str:
    ignore_patterns = r"\b(hmm|mm|mhm|mmm|uh|um|hm|huh|ah|mmhm|hum)\b"
    text = re.sub(ignore_patterns, "", text)
    text = text.replace("…", "")
...
class ContextCollector(torch.utils.data.Dataset):
    ...
    def _get_random_word_lists(self, batch, no_distractors=False, normalize=False):
        ...
        try:
            ...
            distractors = random.choices(  # random choices with replacement
                list(self.rare_words),
                k=distractors_cnt,
            )
        
        # Do some perturbation here on the distractor words
        if self.text_perturbator is not None:
            # TODO: be careful: this is only applicable for spgispeech (lowercase)
            replacement_set = string.ascii_lowercase + string.digits + "'      "  # give it more chance to break the word
    ...
    def get_context_word_list(
        ...
        if self.is_predefined:
            ...
            rare_words_list = self._get_random_word_lists(batch, normalize=normalize)

        # # Oracle word list from slides (no distractors):
        # rare_words_list0 = self._get_predefined_word_lists(batch)
        # rare_words_list1 = self._get_random_word_lists(batch, no_distractors=True)
        # rare_words_list2 = [list(set(r1).intersection(r2)) for r1, r2 in zip(rare_words_list0, rare_words_list1)]
        # # rare_words_list2 = [list(set(r1).intersection(r2)) + ["virus", "brown", "threemonth"] for r1, r2 in zip(rare_words_list0, rare_words_list1)]
        # rare_words_list = rare_words_list2
...
"""
        ...
        rules = rules.lower().strip().split("\n")  # TODO: lower case for librispeech
    ...
    def perturb_texts(self, texts, common_words=[], prob=0.6) -> str:
        ...
        for pos_tag in pos_tags:
            rare_words = [word.lower() for word, tag in pos_tag if tag.startswith("NN") and word not in common_words]
            all_rare_words.append(rare_words)
