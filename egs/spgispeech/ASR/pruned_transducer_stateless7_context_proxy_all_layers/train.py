    ...
    params = AttributeDict(
        {
            ...
            "log_interval": 50,
            "reset_interval": 200,
            "valid_interval": 1200,  # For the 100h subset, use 800
...
def get_contextual_model(params: AttributeDict, decoder=None) -> nn.Module:
    ...
    if params.is_pretrained_context_encoder:
        ...
        if decoder is None:
            ...
            context_encoder = ContextEncoderLSTM(
                ...
                # context_encoder_dim=int(params.encoder_dims.split(",")[-1]),
                # output_dim=params.joiner_dim,
                context_encoder_dim=params.decoder_dim,
                output_dim=context_dim,
                num_layers=2,
                num_directions=2,
                drop_out=0.1,
    ...
    for i, qd in enumerate(params.encoder_dims.split(",")):
        if i in biasing_layers:
            ...
            encoder_biasing_adapters.append(bm)
        else:
            encoder_biasing_adapters.append(None)
    ...
    encoder_biasing_adapters.append(bm)  # The last one is for the outer-most layer, which is not shown in `biasing_layers`
    encoder_biasing_adapters = nn.ModuleList(encoder_biasing_adapters)
...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    ...
    context_encoder, encoder_biasing_adapter, decoder_biasing_adapter = get_contextual_model(params)  # ,decoder
...
def compute_loss(
    ...
    if params.proxy_prob is not None and params.proxy_prob > 0 and is_training and not context_collector.is_predefined and context_collector.text_perturbator is not None:
        ...
        # Simply upsample by twice:
        # texts = new_texts + new_texts
        # batch["supervisions"]["text"] = new_texts + new_texts
        # context_collector.temp_rare_words = new_rare_words + new_rare_words
    else:
        new_texts = None
        new_rare_words = None
    ...
    if True and params.throwaway_prob is not None and is_training:
        ...
        for i, text in enumerate(texts):
            ...
            if num_rare_words == 0 and torch.rand(1) < params.throwaway_prob and feature_lens[i] != feature_lens.max():
                continue
            keep_ids.append(i)
        
        total_chars = sum([len(texts[i]) for i in keep_ids])
        threshold = 35000
    ...
    if False and params.throwaway_prob is not None and is_training:
        ...
        feature_lens = torch.cat((feature_lens, feature_lens), dim=-1)
        texts = texts + new_texts2
        old_texts = old_texts + old_texts
        
        for k in batch['supervisions']:
            if isinstance(batch['supervisions'][k], list):
                batch['supervisions'][k] = batch['supervisions'][k] + batch['supervisions'][k]
            elif isinstance(batch['supervisions'][k], torch.Tensor):
                batch['supervisions'][k] = torch.cat((batch['supervisions'][k], batch['supervisions'][k]), dim=0)
            else:
                raise NotImplementedError
        supervisions = batch["supervisions"]

        if context_collector.temp_rare_words is not None:
            new_rare_words = new_rare_words + new_rare_words2
            context_collector.temp_rare_words = new_rare_words
    ...
    #     breakpoint()
    # !i=3; print(old_texts[i], "\n", new_texts[i], "\n", new_rare_words[i])
    # !len([None for l in new_rare_words if len(l) > 0]), len(new_rare_words), total_chars 
    ...
    with torch.set_grad_enabled(is_training):
        ...
        loss = simple_loss_scale * simple_loss + pruned_loss_scale * pruned_loss + params.ctc_weight * ctc_loss
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        if batch_idx < cur_batch_idx:
            if batch_idx % 200 == 0:
                logging.info(f"Skipping batch {batch_idx}/{cur_batch_idx}")
            continue
...
def run(rank, world_size, args):
    ...
    if params.upsample_N is None:
        ...
        # train_cuts = train_cuts.sample(n_cuts = int(5886320 * 0.3))
    else:
