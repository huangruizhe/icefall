...
def force_alignment(
    ...
    for t in range(T):
        ...
        # decoder_output is of shape (num_active_items, 1, 1, decoder_output_dim)

        current_encoder_out = current_encoder_out.expand(decoder_out.size(0), 1, -1).unsqueeze(1)
        # (num_active_items, 1, 1, encoder_out_dim)
...
def force_alignment_batch(
    ...
    for i in range(N):
        assert 0 < len(ys_list[i]) <= encoder_out_lens[i]
      
    Ts = [encoder_out_lens[packed_encoder_out.sorted_indices[i]].item() for i in range(N)]
    _ys_list = [ys_list[packed_encoder_out.sorted_indices[i].item()] for i in range(N)]
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        hyps_shape_simple = [len(hyps) for hyps in B]
        ...
        decoder_input = torch.cat(
            [
                torch.tensor(
                    hyps.get_decoder_input(
                        ys=ys, context_size=context_size, blank_id=blank_id,
                    ),
                    device=device,
                )
                for hyps, ys in zip(A, _ys_list)
            ],
            dim = 0,
    ...
    sorted_best_alis = [b.topk(1)[0].ys for b in B]
    ans = []
    unsorted_indices = packed_encoder_out.unsorted_indices.tolist()
    for i in range(N):
        ans_i = sorted_best_alis[unsorted_indices[i]]
        assert len(ans_i) == encoder_out_lens[i]
        assert list(filter(lambda i: i != blank_id, ans_i)) == ys_list[i]
        ans.append(ans_i)
...
def get_word_starting_frames2(
    ali: List[int], sp: spm.SentencePieceProcessor, blank_id = 0,
) -> List[int]:
    underscore = b"\xe2\x96\x81".decode()  # '_'
    ans = []
    ...
    ans.append(last_non_blank_idx + 1)
    return ans
