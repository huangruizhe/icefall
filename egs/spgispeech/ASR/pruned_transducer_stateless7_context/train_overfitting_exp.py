...
def compute_loss(
    ...
    if True:
        word_list, word_lengths, num_words_per_utt = \
            context_collector.get_context_word_list(batch, normalize=True)
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if False and batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
...
def run(rank, world_size, args):
    ...
    for param in itertools.chain(
        ...
        # model.context_encoder.parameters(),  # for overfitting exp
    ):
    ...
    ec21_cuts = CutSet.from_file(cut_file)
    ec21_cuts = to_lower_case(ec21_cuts)
    ec21_cuts.describe()
    ...
    train_dl = spgispeech.ec53_dataloaders(ec21_cuts, uid_2_ecid=uid_2_ecid)
    # train_dl = spgispeech.ec53_dataloaders(ec21_cuts, uid_2_ecid=uid_2_ecid, num_buckets=1)
    context_collector._uid_2_ecid = uid_2_ecid

    # valid_cuts = spgispeech.dev_cuts()
    valid_cuts = train_cuts
