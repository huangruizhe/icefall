...
def score(per_utt, spacy_ner_results, common_words):
    wer = WordError()
        ...
        for elem in opcodes:
            ...
            if tag == "equal":
                ...
                if uid in spacy_ner_results_indexed and i1 in spacy_ner_results_indexed[uid]:
                    entity_tag = spacy_ner_results_indexed[uid][i1]
...
def normalize_per_utt(per_utt, spacy_ner_results):
    ...
    for uid in tqdm(per_utt):
        ...
        ref = _normalize_ref(ref, english_normalizer, phrase_mappings, spacy_ner_results, uid)
...
def main(opts):
    ...
    score(per_utt, spacy_ner_results, common_words)
