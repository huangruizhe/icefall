...
class ContextEncoder(torch.nn.Module):
    ...
    def handle_empty_word_list(self, num_words_per_utt):
        device = next(self.rnn.parameters()).device
        no_bias_h = torch.zeros(len(num_words_per_utt), 1, self.output_dim)
        no_bias_h = no_bias_h.to(device)
    ...
    def embed_contexts(
        ...
        if len(word_list) == 0:
            return self.handle_empty_word_list(num_words_per_utt)
