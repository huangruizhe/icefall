...
def decode_one_batch(
    ...
    if not params.no_wfst_lm_biasing:
        ...
        fsa_list, fsa_sizes, num_words_per_utt2 = \
            context_collector.get_context_word_wfst(_batch, cached=True)
    ...
    if not model.no_encoder_biasing or not model.no_decoder_biasing:
        ...
        contexts_h = contexts_h.expand(batch_size, -1, -1)
        contexts_mask = contexts_mask.expand(batch_size, -1)
        ...
        if model.context_encoder.bi_encoders:
            ...
            contexts_dec_h = contexts_dec_h.expand(batch_size, -1, -1)
            contexts_dec_mask = contexts_dec_mask.expand(batch_size, -1)
        else:
            contexts_dec_h, contexts_dec_mask = contexts_h, contexts_mask
        model.scratch_space["contexts_h"] = contexts_dec_h
        model.scratch_space["contexts_mask"] = contexts_dec_mask
    ...
    if not model.no_encoder_biasing:
        encoder_biasing_out, attn = model.encoder_biasing_adapter.forward(encoder_out, contexts_h, contexts_mask, need_weights=True)
        encoder_out = encoder_out + encoder_biasing_out
        model.scratch_space["enc_attn"] = attn
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        for hyp, hyp_raw, ts in zip(sp.decode(hyp_tokens), hyp_tokens, timestamps):
            hyps.append((hyp.split(), [sp.id_to_piece(x) for x in hyp_raw], ts))
...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        torch.save(model.scratch_space["enc_attn"][0].cpu(), '/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_updated4/wordspace/attn_enc.pt')
        torch.save(model.scratch_space["dec_attn"].cpu(), '/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_updated4/wordspace/attn_dec.pt')
        torch.save(context_collector.temp_rare_words_list, '/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_updated4/wordspace/attn_biasing_list.pt')
        breakpoint()
        # !list(zip(*hyps_dict['beam_size_4'][0][1:]))
        # !queries[0][9:15]=queries[0][6]
        # !attn_output[0][3:15]=attn_output[0][6]

        for name, hyps in hyps_dict.items():
            this_batch = []
            assert len(hyps) == len(texts)
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                if isinstance(hyp_words, tuple):
                    hyp_words = hyp_words[0]
                ref_words = text_normalize(ref_text).split()
                hyp_words = text_normalize(" ".join(hyp_words)).split()
...
def rare_word_score(
    ...
    for c in cuts:
        ...
        ref = text_normalize(c.supervisions[0].text)
        batch = {"supervisions": {"text": [ref.lower()]}}
        biasing_words = context_collector._get_random_word_lists(batch, no_distractors=True)
        # refs[uttid] = {"text": ref, "biasing_words": set(biasing_words[0])}
        refs[uttid] = {"text": ref, "biasing_words": set([w.upper() for w in biasing_words[0]])}
...
def main():
    ...
    def to_lower_case(cs):
        new_cs = []
        for c in cs:
            _c = c.to_dict()
            _c["supervisions"][0]["text"] = c.supervisions[0].text.lower()
            _c = MonoCut.from_dict(_c)
            new_cs.append(_c)
        return CutSet.from_cuts(new_cs)

    # cut_file = "/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_json/alignment_sp/segments/cuts_wav.iter10-5.feats.jsonl.gz"
    # cut_file = "/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_json/alignment_sp/segments/cuts_wav.iter10-6.feats.jsonl.gz"
    # cut_file = "/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_updated/alignment/segments/cuts_wav-5.wer.iter4.wer.feats.jsonl.gz"
    cut_file = "/exp/rhuang/meta/audio_ruizhe/ec21/data/earnings21_updated4/timed/raw/segments/ec21_cuts.iter6.wer.updated.feats.jsonl.gz"
    logging.info(f"Loading cuts from: {cut_file}")
    test_cs = CutSet.from_file(cut_file)
    test_cs = to_lower_case(test_cs)
    # test_cs = test_cs.filter(lambda c: c.custom is not None and c.custom["good"] == 1)
    # test_cs = CutSet.from_cuts([test_cs.to_eager()["4330115_140"]])
    # test_cs = CutSet.from_cuts([test_cs.to_eager()["4344338_93"]])  # 4344338_0, 4344338_7
    test_cs = CutSet.from_cuts([test_cs.to_eager()["4344338_9"]])
    # test_cs = CutSet.from_cuts([test_cs.to_eager()["4344866_11"]])
    test_cs.describe()

    # test_dl = uniphore.test_dataloaders(test_cs)
    uid_2_ecid = lambda x: x.split("_")[0]
    # test_dl = spgispeech.ec53_dataloaders(test_cs, uid_2_ecid=uid_2_ecid)
    test_dl = spgispeech.ec53_dataloaders(test_cs, uid_2_ecid=uid_2_ecid, num_buckets=1)
    context_collector._uid_2_ecid = uid_2_ecid

    test_sets = ["ec21"]
    test_dls = [test_dl]
    test_cuts = [test_cs]

    for test_set, test_cut, test_dl in zip(test_sets, test_cuts, test_dls):
