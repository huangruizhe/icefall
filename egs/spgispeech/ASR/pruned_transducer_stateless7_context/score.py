...
def main(args):
    ...
    for uttid in tqdm(refs):
        ...
        if result is None:
            logger.info(f"{uttid} has empty hyp and ref")
            continue
        for code, ref_idx, hyp_idx in zip(result.codes, result.refs, result.hyps):
            if code == Code.match:
                ...
                if ref_tokens[ref_idx] in biasing_words:
                    ...
                    error_details["rare-sub"].append((ref_tokens[ref_idx], hyp_tokens[hyp_idx]))
                else:
                    u_wer.ref_words += 1
                ...
                if ref_tokens[ref_idx] in biasing_words:
                    ...
                    error_details["rare-del"].append((ref_tokens[ref_idx], ""))
                else:
                    u_wer.ref_words += 1
                ...
                if hyp_tokens[hyp_idx] in biasing_words:
                    ...
                    error_details["rare-ins"].append(("", hyp_tokens[hyp_idx]))
                else:
