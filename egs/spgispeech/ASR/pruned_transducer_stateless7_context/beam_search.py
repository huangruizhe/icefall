...
class HypothesisList(object):
    ...
    def get_topk_probable(self, k=None, length_norm: bool = False):
        if length_norm:
            return sorted(self._data.values(), key=lambda hyp: hyp.log_prob / len(hyp.ys), reverse=True)[:k]
        else:
            return sorted(self._data.values(), key=lambda hyp: hyp.log_prob, reverse=True)[:k]
...
def modified_beam_search(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        if hasattr(model, "no_decoder_biasing") and not model.no_decoder_biasing:
            ...
            decoder_biasing_out, attn = model.decoder_biasing_adapter.forward(decoder_out, contexts_h_, contexts_mask_)
            # decoder_biasing_out, attn = model.decoder_biasing_adapter.forward(decoder_out, contexts_h_, contexts_mask_, need_weights=True); attn_list.append(attn)
    ...
    # attn_list_ = [ll[0] for ll in attn_list]
    # attn_list_ = torch.stack(attn_list_, dim=1)
    # model.scratch_space["dec_attn"] = attn_list_
    ...
    if get_topk_hyps != False:
        k = get_topk_hyps
        topk_hyps = [b.get_topk_probable(k=k, length_norm=True) for b in B]
        sorted_ans = [[h.ys[context_size:] for h in hl] for hl in topk_hyps]
        sorted_timestamps = [[h.timestamp for h in hl] for hl in topk_hyps]
        ans = []
        ans_timestamps = []
        unsorted_indices = packed_encoder_out.unsorted_indices.tolist()
        for i in range(N):
