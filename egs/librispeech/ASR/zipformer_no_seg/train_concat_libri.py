...
def convert_long_text_to_fst(items, sp, pid, results):
    ...
    for k, text in tqdm(items, mininterval=2, desc=f"libri_long_text [{pid}]"):
        libri_long_text_sp[k] = make_factor_transducer(sp.encode(text, out_type=int), return_str=True)
...
def make_factor_transducer(word_id_list, return_str=False):
...
def run(rank, world_size, args):
    ...
    model.module.scratch_space["subsampling_factor"] = params.subsampling_factor
    model.module.scratch_space["ctc_beam_size"] = params.ctc_beam_size
