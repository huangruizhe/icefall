...
def decode_one_batch(
    ...
    if not model.no_encoder_biasing:
        encoder_biasing_out, attn = model.encoder_biasing_adapter[-1].forward(encoder_out, contexts_h, contexts_mask)  # , need_weights=True
        encoder_out = encoder_out + encoder_biasing_out
        # breakpoint()
        # i = 2
        # !supervisions["text"][i]
        # !sum(num_words_per_utt[:i]), sum(num_words_per_utt[:i+1])
        # !my_wl = word_list[sum(num_words_per_utt[:i]): sum(num_words_per_utt[:i+1])]
        # !my_wl = [row[:row.ne(0).sum()].tolist() for row in my_wl]
        # !len(my_wl), num_words_per_utt[i]
        # !sp.decode(my_wl)
        # !attn[i].argmax(dim=1)
        # !attn[i].max(dim=1)
