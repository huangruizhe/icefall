...
class AsrModel(nn.Module):
    ...
    def frame_dropout_extended(
        ...
        Returns:
          ...
          new_lengths:
            (N, )
        """
        rd = torch.rand(input_lengths.size(0)) * max_drop_out_rate
        frame_dropout_rate = rd.tolist()
    ...
    def forward(
        ...
        if True:
            max_frame_dropout_rate = 0.2
            if is_training and max_frame_dropout_rate > 0:
                encoder_out_new, encoder_out_lens_new = self.frame_dropout_extended(
                    encoder_out,
                    encoder_out_lens,
                    y_lens,
                    max_frame_dropout_rate,
                ...
                assert torch.all(encoder_out_lens_new <= encoder_out_lens)
                p2d = (0, 0, 0, encoder_out.size(1) - encoder_out_new.size(1))
                encoder_out_new = nn.functional.pad(encoder_out_new, p2d, mode='constant', value=0.0)

                encoder_out = torch.vstack((encoder_out, encoder_out_new))
                encoder_out_lens = torch.hstack((encoder_out_lens, encoder_out_lens_new))
                y = k2.ragged.cat([y, y], axis=0)
                y_lens = torch.hstack((y_lens, y_lens))
