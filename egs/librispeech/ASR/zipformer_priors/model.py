...
class AsrModel(nn.Module):
    ...
    def forward_ctc(
        ...
        # Stat and apply label priors for better alignment
        step_type = args.get("train", None)
        if step_type == "train":
            ...
            if self.log_priors_sum is None:
                args["log_priors_sum"] = log_batch_priors_sum
            else:
                _temp = torch.stack([args["log_priors_sum"], log_batch_priors_sum], dim=-1)
                args["log_priors_sum"] = torch.logsumexp(_temp, dim=-1)
        
        if args.get("log_priors", None) is not None and args["prior_scaling_factor"] > 0:
            log_probs = ctc_output
            log_probs = log_probs - args["log_priors"] * args["prior_scaling_factor"]
            ctc_output = log_probs
