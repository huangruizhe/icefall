...
class Zipformer2(EncoderInterface):
    ...
    def forward(
        ...
        # Note to myself:
        # 1) throughout the encoder layers, the lengths of time axis are unchanged
        #     (Pdb) !outputs[-1].shape
        #     torch.Size([870, 45, 256])
        #     (Pdb) !outputs[-2].shape
        #     torch.Size([870, 45, 384])
        #     (Pdb) !outputs[-3].shape
        #     torch.Size([870, 45, 512])
        #     (Pdb) !outputs[-4].shape
        #     torch.Size([870, 45, 384])
        #     (Pdb) !outputs[-5].shape
        #     torch.Size([870, 45, 256])
        #     (Pdb) !outputs[-6].shape
        #     torch.Size([870, 45, 192])
        # 2) the lengths of time axis are halved after the last encoder layer due to the `downsample_output`
        # 3) don't worry about `_get_full_dim_output`, it's just a concatenation of outputs along the last dimension to make D=512 for the final output
        x_intermediate = self.downsample_intermediate_output(outputs[self.i_intermediate])
        ...
        return x, lengths, x_intermediate
