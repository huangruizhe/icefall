...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ctc_output, encoder_memory, memory_mask = model(
            feature, supervisions, warmup=warmup
        )
        # ctc_output is (N, T, C)
        # ctc_output is a logsoftmax probability
    ...
    if True and is_training:
        ...
        batch_wer = get_batch_wer(
            params, 
            ctc_output, 
            batch,
            sp,
            decoding_graph=y_long,
        )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if params.batch_idx_train > 0 and params.batch_idx_train % params.valid_interval == 0:
