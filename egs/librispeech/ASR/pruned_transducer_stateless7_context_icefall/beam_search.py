...
def modified_beam_search(
    ...
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    context_graphs: Optional[List[ContextGraph]] = None,
    beam: int = 4,
    temperature: float = 1.0,
    blank_penalty: float = 0.0,
    return_timestamps: bool = False,
    ...
    for i in range(N):
        B[i].add(
            Hypothesis(
                ...
                context_state=None if context_graphs is None else (context_graphs[i], context_graphs[i].root),
                timestamp=[],
            )
        )
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                context_score = 0
                if hyp.context_state is not None:
                    my_context_graph, my_context_state = hyp.context_state
                else:
                    my_context_graph, my_context_state = None, None
                new_context_state = None if context_graphs is None else my_context_state
                if new_token not in (blank_id, unk_id):
                    ...
                    if context_graphs is not None:
                        (
                            context_score,
                            new_context_state,
                        ) = my_context_graph.forward_one_step(my_context_state, new_token)
                ...
                new_hyp = Hypothesis(
                    ...
                    context_state=(my_context_graph, new_context_state),
                )
                B[i].add(new_hyp)
    ...
    if context_graphs is not None:
        ...
        for i, hyps in enumerate(B):
            for hyp in list(hyps):
                my_context_graph, my_context_state = hyp.context_state
                context_score, new_context_state = my_context_graph.finalize(
                    my_context_state
                )
                finalized_B[i].add(
                    Hypothesis(
