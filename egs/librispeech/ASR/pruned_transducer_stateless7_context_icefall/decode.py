    ...
    for batch_idx, batch in enumerate(dl):
        ...
        if context_file is not None:
            ...
            for cut in batch['supervisions']['cut']:
                ...
                if uid in context_file:
                    contexts = context_file[uid]
                else:
                    contexts = []
...
def rare_word_score(
    ...
    if params.n_distractors > 0:
        args.refs = params.context_file + f"/ref/{test_set_name}.biasing_{params.n_distractors}.tsv"
    else:
        args.refs = params.context_file + f"/ref/{test_set_name}.biasing_{params.n_distractors}.tsv"
    args.lenient = True
...
def main():
    ...
    if params.decoding_method == "modified_beam_search":
        if os.path.exists(params.context_file):
            ...
            for part in ["test-clean", "test-other"]:
                _context_file, _ = read_ref_biasing_list(params.context_file + f"/ref/{part}.biasing_{params.n_distractors}.tsv")
                context_file[part] = _context_file
        else:
            context_file = None
    else:
        context_file = None
    ...
    for test_set, test_dl in zip(test_sets, test_dl):
        results_dict = decode_dataset(
            ...
            ngram_lm_scale=ngram_lm_scale,
            LM=LM,
            context_file=context_file[test_set],
        )
