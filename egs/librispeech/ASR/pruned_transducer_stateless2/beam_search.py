...
from icefall import BiasedNgramLm, BiasedNgramLmStateBonus, NgramLm, NgramLmStateCost
...
def modified_beam_search_rnnlm_shallow_fusion_biased(
    model: Transducer,
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    sp: spm.SentencePieceProcessor,
    biased_lm: NgramLm,
    biased_lm_scale: float,
    rnnlm: RnnLmModel,
    rnnlm_scale: float,
    beam: int = 4,
    return_timestamps: bool = False,
    ...
    for i in range(N):
        B[i].add(
            Hypothesis(
                ...
                lm_score=init_score.reshape(-1),
                timestamp=[],
                state_cost=BiasedNgramLmStateBonus(biased_lm),
            )
        )
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        ys_log_probs = torch.cat(
            [
                hyp.log_prob.reshape(1, 1) + hyp.state_cost.lm_score * biased_lm_scale
                for hyps in A
                for hyp in hyps
            ]
        )
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                hyp_log_prob -= hyp.state_cost.lm_score * biased_lm_scale
